...
public static class SeriesStoredProcedureModule
{
    public static void AddSeriesStoredProcedureEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapPost("/stored-procedure-create-series",
            async ([FromBody] CreateSeriesRequest createSeriesRequest, [FromServices] AppDbcontext dbContext) =>
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
                    $"EXEC CreateSeries @GenreId={createSeriesRequest.Genre_Id}, @Title={createSeriesRequest.Title}, @AgeRating={createSeriesRequest.Age_Rating}");
                return Results.Ok();
            });
        ...
        app.MapGet("/stored-procedure-get-episodes-by-series-id/{seriesId:int}",
            async (int seriesId, [FromServices] AppDbcontext dbContext) =>
            {
                ...
                if (series == null) return Results.NotFound(new { message = "Series not found." });
        ...
        app.MapGet("/stored-procedure-get-series-by-id/{seriesId:int}",
            async (int seriesId, [FromServices] AppDbcontext dbContext) =>
            {
                var series = await dbContext.Series
                    .FromSqlInterpolated($"EXEC GetSeriesById @SeriesId={seriesId}")
                    .ToListAsync()
                    .ContinueWith(task => Enumerable.Select(task.Result, s => new SeriesDto
                    {
        ...
        app.MapPut("/stored-procedure-update-series-by-id", async ([FromBody] UpdateSeriesRequest updateSeriesRequest,
            [FromServices] AppDbcontext dbContext) =>
        {
            await dbContext.Database.ExecuteSqlInterpolatedAsync(
                $"EXEC UpdateSeriesById @SeriesId={updateSeriesRequest.Series_Id}, @GenreId={updateSeriesRequest.Genre_Id}, @Title={updateSeriesRequest.Title}, @AgeRating={updateSeriesRequest.Age_Rating}");
            return Results.Ok();
        });

        app.MapDelete("/stored-procedure-delete-series/{seriesId}",
            async (int seriesId, [FromServices] AppDbcontext dbContext) =>
            {
                try
                {
