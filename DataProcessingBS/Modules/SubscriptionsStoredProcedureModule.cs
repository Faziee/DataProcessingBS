...
public static class SubscriptionsStoredProcedureModule
{
    public static void AddSubscriptionStoredProcedureEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapPost("/stored-procedure-create-subscription",
            async ([FromBody] CreateSubscriptionRequest createSubscriptionRequest,
        ...
        app.MapGet("/stored-procedure-get-subscription-by-id/{subscriptionId:int}",
            async ([FromServices] AppDbcontext dbContext, int subscriptionId) =>
            {
                var subscriptions = await dbContext.Set<SubscriptionDto>()
        ...
        app.MapPut("/stored-procedure-update-subscription-by-id",
            async ([FromBody] UpdateSubscriptionRequest updateSubscriptionRequest,
                [FromServices] AppDbcontext dbContext) =>
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
        ...
        app.MapDelete("/stored-procedure-delete-subscription-by-id/{subscriptionId}",
            async (int subscriptionId, [FromServices] AppDbcontext dbContext) =>
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
                    $"EXEC DeleteSubscriptionById @SubscriptionId={subscriptionId}");
                return Results.Ok();
            });
    }
}